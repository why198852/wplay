...
object RedisConnectionPool extends Logging {
  ...
  def safeClose[R](f: Jedis => R)(implicit jedis: Jedis): R = {
    ...
    Try {
      ...
      case Success(o) => logDebug("jedis.close successful.")
      case Failure(o) => logError("jedis.close failed.")
    }
    result
  }
  ...
  def safeClosePipe[R](f: Pipeline => R)(implicit jedis: Jedis): R = {
    ...
    Try {
      ...
      case Success(o) => logDebug("pipe.close successful.")
      case Failure(o) => logError("pipe.close failed.")
    }
    result
  }
}
