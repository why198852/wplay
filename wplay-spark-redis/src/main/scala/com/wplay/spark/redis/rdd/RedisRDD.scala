package com.wplay.spark.redis.rdd
...
import com.wplay.spark.redis.{RedisConfig, RedisNode}
import com.wplay.spark.redis.partitioner.RedisPartition
...
class RedisKeysRDD(sc: SparkContext,
  ...
  private def scaleHostsWithPartitionNum(): Seq[(String, Int, Int, Int)] = {
    ...
    if (hosts.length == partitionNum) {
      ...
      val lastExtCnt = if (presExtCnt * hosts.length < partitionNum) presExtCnt + 1 else presExtCnt
      hosts.zipWithIndex.flatMap{
        case(host, idx) =>
          split(host, if (idx == hosts.length - 1) lastExtCnt else presExtCnt)
      }
    } else {
      val presExtCnt = hosts.length / partitionNum
 ...
 *
  ...
  def getZSet: RDD[String] = {
    val zsetContext: ZSetContext = ZSetContext(0, -1, Double.MinValue, Double.MaxValue, withScore = false, "byRange")
 ...
 *
  ...
  def getZSetWithScore: RDD[(String, Double)] = {
    val zsetContext: ZSetContext = ZSetContext(0, -1, Double.MinValue, Double.MaxValue, withScore = true, "byRange")
 ...
 *
  ...
  def getZSetByRange(startPos: Long, endPos: Long): RDD[String] = {
    val zsetContext: ZSetContext = ZSetContext(startPos, endPos, Double.MinValue, Double.MaxValue, false, "byRange")
    new RedisZSetRDD(this, zsetContext, classOf[String])
  }
  /**
 ...
 *
  ...
  def getZSetByRangeWithScore(startPos: Long, endPos: Long): RDD[(String, Double)] = {
    val zsetContext: ZSetContext = ZSetContext(startPos, endPos, Double.MinValue, Double.MaxValue, true, "byRange")
    new RedisZSetRDD(this, zsetContext, classOf[(String, Double)])
  }
  /**
 ...
 *
  ...
  def getZSetByScore(min: Double, max: Double): RDD[String] = {
    val zsetContext: ZSetContext = ZSetContext(0, -1, min, max, false, "byScore")
    new RedisZSetRDD(this, zsetContext, classOf[String])
  }
  /**
 ...
 *
  ...
  def getZSetByScoreWithScore(min: Double, max: Double): RDD[(String, Double)] = {
    val zsetContext: ZSetContext = ZSetContext(0, -1, min, max, true, "byScore")
    new RedisZSetRDD(this, zsetContext, classOf[(String, Double)])
  }
}
  ...
  private def isRedisRegex(key: String) = {
    def judge(key: String, escape: Boolean): Boolean = {
      if (key.length == 0) {
        ...
        escape match {
          case true => judge(key.substring(1), escape = false)
          case false => key.charAt(0) match {
            case '*'  => true
            case '?'  => true
            case '['  => true
            case '\\' => judge(key.substring(1), escape = true)
            case _    => judge(key.substring(1), escape = false)
          }
        }
      }
    }
    judge(key, escape = false)
  }
